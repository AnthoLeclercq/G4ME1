    ...
    SubShader {
        ...
        Pass {
            ...
            UNITY_INSTANCING_BUFFER_START( Props )
                UNITY_DEFINE_INSTANCED_PROP( float, _ScanlineDensity)
                UNITY_DEFINE_INSTANCED_PROP( float, _ScanlineSpeed)
                UNITY_DEFINE_INSTANCED_PROP( float, _ScanlineExp)
                UNITY_DEFINE_INSTANCED_PROP( float, _DesatAmount)
                UNITY_DEFINE_INSTANCED_PROP( float, _Brightness)
                UNITY_DEFINE_INSTANCED_PROP( float, _ScanlineOpacity)
                UNITY_DEFINE_INSTANCED_PROP( float, _ObjectOpacity)
                UNITY_DEFINE_INSTANCED_PROP( float4, _Color)
                UNITY_DEFINE_INSTANCED_PROP( float, _ColorIntensity)
                UNITY_DEFINE_INSTANCED_PROP( fixed, _YAxis)
                UNITY_DEFINE_INSTANCED_PROP( float, _FresIntensity)
                UNITY_DEFINE_INSTANCED_PROP( float, _IntersectAttenuation)
                UNITY_DEFINE_INSTANCED_PROP( float, _IntersectBrightness)
...
////// Lighting:
////// Emissive:
                float _Brightness_var = UNITY_ACCESS_INSTANCED_PROP( Props, _Brightness ); // Diffuse Brightness
                float4 _Diffuse_var = tex2D(_Diffuse,TRANSFORM_TEX(i.uv0, _Diffuse));
                float _DesatAmount_var = UNITY_ACCESS_INSTANCED_PROP( Props, _DesatAmount );
                float4 _Color_var = UNITY_ACCESS_INSTANCED_PROP( Props, _Color );
                float _ColorIntensity_var = UNITY_ACCESS_INSTANCED_PROP( Props, _ColorIntensity );
                float _IntersectAttenuation_var = UNITY_ACCESS_INSTANCED_PROP( Props, _IntersectAttenuation ); // Glow area where objects intersect
                float _IntersectBrightness_var = UNITY_ACCESS_INSTANCED_PROP( Props, _IntersectBrightness );
                float _FresIntensity_var = UNITY_ACCESS_INSTANCED_PROP( Props, _FresIntensity ); // Amount of edge glow
                float4 node_1332 = _Time;
                float _ScanlineSpeed_var = UNITY_ACCESS_INSTANCED_PROP( Props, _ScanlineSpeed );
                float2 _YAxis_var = lerp( float2(i.posWorld.g,i.posWorld.r), float2(i.posWorld.r,i.posWorld.g), UNITY_ACCESS_INSTANCED_PROP( Props, _YAxis ) );
                float _ScanlineDensity_var = UNITY_ACCESS_INSTANCED_PROP( Props, _ScanlineDensity ); // Number of Scanlines
                float _ScanlineExp_var = UNITY_ACCESS_INSTANCED_PROP( Props, _ScanlineExp ); // Exposure of the scanlines
                float _ScanlineOpacity_var = UNITY_ACCESS_INSTANCED_PROP( Props, _ScanlineOpacity );
                float3 node_5793 = (pow(frac((1.0 - ((node_1332.r*float2(0.0,_ScanlineSpeed_var))+(_YAxis_var*float2(1.0,_ScanlineDensity_var)))).g),_ScanlineExp_var)*_ScanlineOpacity_var*_Color_var.rgb);
                float _ObjectOpacity_var = UNITY_ACCESS_INSTANCED_PROP( Props, _ObjectOpacity );
                float2 node_2691 = ((sceneUVs * 2 - 1).rg*node_1332.r);
                ...
                float _StaticIntensity_var = UNITY_ACCESS_INSTANCED_PROP( Props, _StaticIntensity );
                float3 emissive = (((_Brightness_var*(lerp(_Diffuse_var.rgb,dot(_Diffuse_var.rgb,float3(0.3,0.59,0.11)),_DesatAmount_var)*(_Color_var.rgb*_ColorIntensity_var)*_Color_var.rgb)*((1.0 - saturate((sceneZ-partZ)/_IntersectAttenuation_var))+_IntersectBrightness_var+pow(1.0-max(0,dot(normalDirection, viewDirection)),(1.0 - _FresIntensity_var))))+node_5793)*(node_5793+_ObjectOpacity_var+lerp(0.0,((node_8683*0.5+0.5)*_StaticIntensity_var),_StaticIntensity_var)));
                float3 finalColor = emissive;
                fixed4 finalRGBA = fixed4(finalColor,(_Diffuse_var.a*_ObjectOpacity_var));
        ...
        Pass {
            ...
            float4 frag(VertexOutput i, float facing : VFACE) : SV_Target {
                ...
                float node_8683 = frac(node_8683_rnd.x*node_8683_rnd.y*(1+node_8683_skew.x)); // Static effect
                float _StaticIntensity_var = UNITY_ACCESS_INSTANCED_PROP( Props, _StaticIntensity );
                o.Emission = (((_Brightness_var*(lerp(_Diffuse_var.rgb,dot(_Diffuse_var.rgb,float3(0.3,0.59,0.11)),_DesatAmount_var)*(_Color_var.rgb*_ColorIntensity_var)*_Color_var.rgb)*((1.0 - saturate((sceneZ-partZ)/_IntersectAttenuation_var))+_IntersectBrightness_var+pow(1.0-max(0,dot(normalDirection, viewDirection)),(1.0 - _FresIntensity_var))))+node_5793)*(node_5793+_ObjectOpacity_var+lerp(0.0,((node_8683*0.5+0.5)*_StaticIntensity_var),_StaticIntensity_var)));
